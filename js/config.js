...
// ★ 地域別赤色立体地図（REGIONAL_CS_LAYERS から dem2cs:// → dem2rrim:// に置換して派生）
// 事前生成タイル（https:// 始まり）は変換不可のため除外。
export const REGIONAL_RRIM_LAYERS = REGIONAL_CS_LAYERS
  .filter(l => l.tileUrl.startsWith('dem2cs://'))
  .map(l => ({
    ...l,
    sourceId: l.sourceId.replace('cs-', 'rrim-'),
    layerId:  l.layerId.replace('cs-', 'rrim-'),
    tileUrl:  l.tileUrl.replace('dem2cs://', 'dem2rrim://'),
    label:    l.label.replace('CS立体図', '赤色立体図'),
  }));
